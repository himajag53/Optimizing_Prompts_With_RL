...
class LMAdaptorModel(BaseModel):
    ...
    def __init__(
        ...
        if task == 'question-answering':
            self.generator_last_layer = self.generator.model.qa_outputs
        else:
            self.generator_last_layer = self.generator.model.lm_head
    ...
    def _mlp_forward(self, state: torch.Tensor) -> torch.Tensor:
        ...
        logits = self.generator_last_layer(mlp_output)

        if self.fluent:
            lm_logits = self.generator_last_layer.model(state)
    ...
    def sample(
        ...
        **kwargs
        ...
        sample_ids = torch.cat(sample_ids, dim=1)        # [batch_size, prompt_length, vocab_size]
        sample_logits = torch.cat(sample_logits, dim=1)
